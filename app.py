...
def transactions_entry_page():
    ...
    st.subheader("💰 Paid / Pending / Partial Summary")
    status_group = df_user.groupby("status").agg({"amount":"sum"}).reset_index()
    st.dataframe(status_group)
    if not status_group.empty:
        st.bar_chart(status_group.set_index("status")["amount"])
...
def suppliers_entry_page():
    ...
    st.subheader("💰 Paid / Pending / Partial Summary")
    status_group = df_user[["paid_amt","pending_amt","partial_amt"]].sum().reset_index()
    status_group.columns = ["Status","Amount"]
    st.dataframe(status_group)
    if not status_group.empty:
        st.bar_chart(status_group.set_index("Status")["Amount"])
...
def reports_page():
    ...
    st.subheader("💹 Charts (Streamlit)")
    if not prod_group.empty:
        st.bar_chart(prod_group.set_index("service_type")[["num_apps","paid_amt","profit_amt"]])
